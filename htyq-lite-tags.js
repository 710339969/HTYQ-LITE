// htyq-lite-tags.js — 预测标签生成（四层流水线 v2.2.0）
// ============================================================
// ★ 修改记录 ★
// 2026-06-05 v2.1.0
//   - 增强实体/地点/势力检测模式
...
window.HTYQ_LITE_TAGS = (function() {
  ...
  async function extractByAI(chatHistory, state) {
    ...
    if (!evolution || typeof evolution.callApi !== 'function') {
      console.warn('[HTYQ Tags] 演化模块不可用，跳过AI标签提取');
      return null;
    }
...
["林月如", "location:蜀山", "faction:蜀山派", "topic:修行", "emotion:温馨"]`;
  ...
  async function generatePredictionTags(chatHistory, worldState) {
    ...
    try {
      ...
      console.warn('[HTYQ Tags] AI标签提取失败，降级规则', e.message);
    }
    ...
    console.log(`[HTYQ Tags] 预测标签 (${finalTags.length}个):`, finalTags);
    return finalTags;
  }
