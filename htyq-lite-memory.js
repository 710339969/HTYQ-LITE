// htyq-lite-memory.js — 记忆提取、存储、三层摘要、召回、情感
// ============================================================
// ★ 修改记录 ★
// 2026-06-05 v2.1.0
...
window.HTYQ_LITE_MEMORY = (function() {
  ...
  async function generateSmartSummaryWithTags(userMsg, aiMsg, round, locationHint = null) {
    ...
    if (!callApi) {
      console.warn('[HTYQ] 无法获取 callApi，智能摘要不可用');
      return null;
    }
...
- 每个数组最多填5个最相关的标签，如果没有就留空数组 []。`;
    ...
    try {
      ...
      console.error('[HTYQ] 智能摘要+标签提取失败:', e);
      return null;
    }
  }
...
[设伏]：李四将情报传递给官府，在青石关设下埋伏。`;
  ...
  function extractEmotion(text, entities) {
    ...
    for (const entity of entities) {
      ...
      if (!isPotentialNpc(entity)) {
        console.log('[HTYQ Emotion] 跳过非NPC实体:', entity);
        continue;
      }
  ...
  async function storeMemoryFromRound(state, userMsg, aiMsg, round) {
    ...
    if (useSmart) {
      try {
        ...
        console.warn('[HTYQ] 智能摘要失败，回退规则', e);
        tags = extractTagsByRules(combined);
        summary = generateRuleBasedSummary(userMsg, aiMsg, round, tags);
        importance = calculateImportance(combined, tags);
      }
    ...
    console.log(`[HTYQ Memory] 存储记忆 (重要性${importance}): ${summary.substring(0, 80)}...`);
    ...
    for (const em of emotions) {
      ...
      if (oldAttitude !== em.attitude) {
        console.log(`[HTYQ Emotion] 情感变化: ${em.entity} ${oldAttitude} → ${em.attitude} (${em.reason})`);
        ...
        if (em.attitude === '不共戴天' && (oldAttitude === '友善' || oldAttitude === '信任')) {
          if (!state.bloodFeudMemo.some(b => b.faction === em.entity)) {
            ...
            console.log(`[HTYQ Emotion] 血仇记录新增: ${em.entity}`);
          }
        }
      }
      ...
      if (isPotentialNpc(em.entity)) {
        ...
        console.log('[HTYQ Emotion] 二次过滤跳过:', em.entity);
      }
    }

    
      // ★ v3.1.2: keyword combat detection fallback
      if (window.HTYQ_LITE_CORE && typeof window.HTYQ_LITE_CORE.addCombatLog === "function" && state) {
        ...
        if (hasBattle) {
          ...
          window.HTYQ_LITE_CORE.addCombatLog(state, {
          ...
          console.log("[HTYQ Combat] keyword detected: " + killCnt + " kills, " + outcome);
        }
      }
      return memory;
  }
  ...
  async function mergeChapterSummary(state, startRound, endRound) {
    ...
    if (settings.smartTagging !== false) {
      ...
      } catch(e) { console.warn('[HTYQ] 生成章节结构化事件失败', e); }
    }
    ...
    console.log(`[HTYQ Memory] 章节摘要已生成 (${startRound}-${endRound})，保留${memories.length - toDelete.length}条核心记忆`);
  }
  ...
  async function mergeVolumeSummary(state, startRound, endRound) {
    ...
    if (settings.smartTagging !== false) {
      ...
      } catch(e) { console.warn('[HTYQ] 生成卷结构化事件失败', e); }
    }
    ...
    core.saveState(state);
    console.log(`[HTYQ Memory] 卷摘要已生成 (${startRound}-${endRound})`);
  }
