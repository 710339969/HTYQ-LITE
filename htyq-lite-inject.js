// htyq-lite-inject.js — 构建注入上下文（记忆+面板状态+世界书[可选]），支持当前地点和活跃实体权重，增加长度限制
...
window.HTYQ_LITE_INJECT = (function() {
  ...
  async function buildContext(chatHistory, worldState, tags, options = {}) {
    // ★ v2.5.0: 加载预设
    const presets = window.HTYQ_LITE_PRESETS;
    ...
    for (var bi = 0; bi < blockMeta.length; bi++) {
      ...
      if (budget < 2000) console.debug('[HTYQ] token预算紧张:', budget);
      if (nextTotal > budget) {
        // 预算不足，跳过此块及后续所有低优先级块
        continue;
      }
    ...
    return finalContext;
      ...
      console.error('[HTYQ] buildContext 模块异常', e);
      return '[HTYQ Lite] 世界状态构建异常，已降级';
    }
  }
