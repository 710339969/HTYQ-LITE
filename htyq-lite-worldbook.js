...
//   - 新增 getActiveEntryMap() / setActiveEntryMap() — localStorage 持久化用户选择
...
window.HTYQ_LITE_WORLDBOOK = (function() {
  ...
  // ---------- 存储键名 ----------
  const SELECTION_KEY = 'htyq_lite_worldbook_selection';
  const BOOK_SEL_KEY = 'htyq_lite_wb_books';
  ...
  async function getAvailableBooks() {
    ...
    } catch(e) { console.warn('[HTYQ WB] 获取角色世界书列表失败', e); }
    ...
    } catch(e) { console.warn('[HTYQ WB] 获取全局世界书列表失败', e); }
    ...
    } catch(e) { console.warn('[HTYQ WB] 合并世界书库失败', e); }
      ...
      if (resp.ok) {
        ...
        for (const name of worldNames) {
          try {
            ...
            console.warn(`[HTYQ WB] 世界书 "${name}" 加载失败:`, e);
          }
        }
      }
    } catch(e) { console.warn('[HTYQ WB] 获取全部世界书失败', e); }
    return result;
  }
  ...
  async function getBookEntries(bookName) {
    ...
    try {
      ...
      console.warn(`[HTYQ WB] 获取世界书条目失败: ${bookName}`, e);
      return [];
    }
  }
  ...
  function setActiveEntryMap(selectionMap) {
    ...
    try {
      localStorage.setItem(SELECTION_KEY, JSON.stringify(selectionMap));
      console.log('[HTYQ WB] 世界书选择已保存:', Object.keys(selectionMap).length, '本书');
    } catch(e) {
      console.warn('[HTYQ WB] 保存世界书选择失败', e);
    }
  }
  ...
  function getBookSelection() {
    try {
      const raw = localStorage.getItem(BOOK_SEL_KEY);
  ...
  function setBookSelection(selectionMap) {
    ...
    try {
      localStorage.setItem(BOOK_SEL_KEY, JSON.stringify(selectionMap));
      console.log('[HTYQ WB] 书级选择已保存:', Object.keys(selectionMap).length, '本书');
    } catch(e) {
      console.warn('[HTYQ WB] 保存书级选择失败', e);
    }
  }

  function clearBookSelection() {
    localStorage.removeItem(BOOK_SEL_KEY);
  }
  ...
  async function loadWorldbooks() {
    ...
    } catch(e) { console.warn('[HTYQ WB] 加载用户选中世界书失败', e); }
    ...
    } catch(e) { console.warn('[HTYQ WB] 加载部分选择世界书失败', e); }
    ...
    console.log(`[HTYQ Worldbook] 加载完成，共 ${entries.length} 个条目，${bookMetaCache.length} 本书`);
    return entries;
  }
  ...
  async function analyzeWorldbooks(bookNames) {
    ...
    try {
      ...
      console.warn('[HTYQ WB] analyzeWorldbooks 调用失败', e);
      return null;
    }
  }
  ...
  async function analyzeText(rawText) {
    ...
    try {
      ...
      console.warn('[HTYQ WB] analyzeText 调用失败', e);
      return null;
    }
  }
